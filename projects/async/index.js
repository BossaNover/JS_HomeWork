...
function isMatching(full, chunk) {
  return full.toLowerCase().includes(chunk.toLowerCase()); //проверка на включение куска (chunk) в полном слове
}
...
retryButton.addEventListener('click', () => {
  //повтор загрузки городов
  tryToLoad();
});

filterInput.addEventListener('input', function () {
  //при каждом изменении текстового поля вызывается функция
  updateFilter(this.value);
});

loadingFailedBlock.classList.add('hidden'); //скрываем блок, т.к. вначале нет ошибки
filterBlock.classList.add('hidden'); //скрываем блок, т.к. вначале непонятно, загрузился список или нет
...
function updateFilter(filterValue) {
  filterResult.innerHTML = ''; //ЗАЧЕМ?

  const fragment = document.createDocumentFragment(); //создание фрагмента DOM (т.е.много элементов???)
  //когда DocumentFragment вставляется в DOM – то он исчезает, а вместо него вставляются его дети

  for (const town of towns) {
    /// в цикле происходит наполнение фрагмента элементами div
